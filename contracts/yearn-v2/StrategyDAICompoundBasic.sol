...
contract StrategyDAICompoundBasic {
    ...
    // Comptroller address for Compound
    ...
    function deposit() public {
        ...
        if (_want > 0) {
            ...
            cToken(cDAI).mint(_want); // Interface CToken
        }
    }
    ...
    function _withdrawAll() internal {
        uint256 amount = balanceC(); // IERC20(cDAI).balanceOf(address(this));
        if (amount > 0) {
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
        ...
        uint256 amount = ((b * _amount) / bT) + 1; // amount = amount / cToken(cDAI).exchangeRateStored();
        uint256 _before = IERC20(want).balanceOf(address(this));
        _withdrawC(amount); // cToken(cDAI).redeem(amount);
    ...
    function balanceCInToken() public view returns (uint256) {
        ...
        uint256 b = balanceC();
        if (b > 0) {
            b = b * (cToken(cDAI).exchangeRateStored() / 1e18); // Amount * exchangeRateStored()
        }
        return b;
    }
